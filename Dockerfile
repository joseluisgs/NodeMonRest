...
# Luego, creamos a imagen: docker build -t joseluisgs/nodemonrest .  (oye que lo del punto no está mal, es que es así :))
# Luego: docker run -p 49160:8000 -d joseluisgs/nodemonest de esta manera exponemos los puertos. en nuestra maquina es el 49160, que se mapea al 8000 del docker
# Luego: docker ps
